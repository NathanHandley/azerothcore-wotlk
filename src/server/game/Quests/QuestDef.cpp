...
uint32 Quest::XPValue(uint8 playerLevel) const
{
    ...
    if (sWorld->getIntConfig(CONFIG_MIN_SCALED_XP_RATIO_QUEST))
    {
        uint32 minScaledXP = RoundXPValue(xpentry->Exp[RewardXPDifficulty]) * sWorld->getIntConfig(CONFIG_MIN_SCALED_XP_RATIO_QUEST) / 100;
