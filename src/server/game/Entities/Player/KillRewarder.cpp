...
void KillRewarder::_InitGroupData()
{
    if (_group)
    {
            ...
            if (Player* member = itr->GetSource())
                if ((_killer == member || member->IsAtGroupRewardDistance(_victim)))
                {
                    ...
                    if (member->IsAlive())
                    {
                        ...
                        if (sWorld->getIntConfig(CONFIG_MIN_SCALED_XP_RATIO_KILL) > 0)
                        {
        ...
        // 2.6. _isFullXP - flag identifying that for all group members victim is not gray,
        //      so 100% XP will be rewarded (50% otherwise).
        if (sWorld->getIntConfig(CONFIG_MIN_SCALED_XP_RATIO_KILL) == 100)
            _isFullXP = true;
        else
...
void KillRewarder::_RewardXP(Player* player, float rate)
{
    ...
    if (_group)
    {
        ...
        if (rewardProportion < sWorld->getIntConfig(CONFIG_MIN_SCALED_XP_RATIO_KILL))
            rewardProportion = sWorld->getIntConfig(CONFIG_MIN_SCALED_XP_RATIO_KILL);
